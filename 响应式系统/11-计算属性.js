...
function flushJob() {
  ...
  // todo: 这里需要注意的是这里是一个微任务队列，所以也就只会执行一次了
  p.then(() => {
...
function computed(getter) {
  // value 用来缓存上一次计算的值
  let value
  // dirty 标志，用来标识是否需要重新计算，为 true 时则意味着脏，需要计算
  let dirty = true
...
obj.foo++
// 再次访问还是3，因为 dirty 在第一次访问后就永远是false了，
// 我们需要修改为在 obj.foo 或者 obj.bar 发生变化后重新设置 dirty 的值，所以需要用到之前写的调度器
console.log('sumRes.value', sumRes.value)
